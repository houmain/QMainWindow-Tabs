...
#else
        const auto position = event->globalPos();
#endif
        openTabsList(position);
    }
}
...
void DockTitle::contextMenuEvent(QContextMenuEvent *event)
{
    const auto index = tabContainingPoint(event->pos());
    if (index >= 0) {
        Q_EMIT contextMenuRequested(event->globalPos(), mTabBar, tabDock(index));
    }
}
